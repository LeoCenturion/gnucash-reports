...
(define (make-classification-table report-obj)
  (let* ((table (gnc:make-html-table))
         ...
         (filter-negative (lambda (accs) (filter (lambda (acc-name) (negative? (gnc:gnc-monetary-amount (account-balance->monetary acc-name init-date end-date)))) accs)))
         ...
         (debt-paid (sum-accounts (filter-negative (debt-account-list)))))
    ...
    (gnc:html-table-append-row! table (list "Debt paid" debt-paid))
    (gnc:html-table-append-row! table (list "Total" (gnc:monetary+ debt-paid
